    ...
    def build_from_warm_up_log(self, input_strategy_library, input_summarizer_log):
        ...
        for request_id in range(len(self.data['warm_up'])):
            ...
            if score_j > score_i:
                ...
                while not_scussed:
                    try:
                        strategy, summarizer_system = self.summarizer.summarize(request, jailbreak_prompt_i, jailbreak_prompt_j, current_library, max_tokens=10000, do_sample=True, temperature=0.6, top_p=0.9)
                        json_formatted_strategy = self.summarizer.wrapper(strategy, max_tokens=2000, do_sample=True, temperature=0.6, top_p=0.9)
                ...
                for j in range(self.epochs):
                    ...
                    while not success:
                        try:
                            ...
                            score = self.scorer.wrapper(assessment, max_tokens=2000, do_sample=False)
                            success = True
                ...
                for j in range(self.epochs):
                    ...
                    if j == 0:
                        ...
                        if retrival_strategy_list:
                            if valid:
                                jailbreak_prompt, attacker_system = self.attacker.use_strategy(request, retrival_strategy_list, max_tokens=10000, do_sample=True, temperature=1.0, top_p=1)
                            else:
                                jailbreak_prompt, attacker_system = self.attacker.find_new_strategy(request, retrival_strategy_list, max_tokens=10000, do_sample=True, temperature=1.0, top_p=1)
                        else:
                            jailbreak_prompt, attacker_system = self.attacker.warm_up_attack(request, max_tokens=10000, do_sample=True, temperature=1.0, top_p=1)
                        for word in self.refusal_list:
                            if word in jailbreak_prompt:
                                jailbreak_prompt = request
                        target_response = self.target.respond(jailbreak_prompt)
                        ...
                        while not success:
                            try:
                                assessment, scorer_system = self.scorer.scoring(request, target_response, max_tokens=10000, do_sample=True, temperature=0.7, top_p=0.9)
                                score = self.scorer.wrapper(assessment, max_tokens=2000, do_sample=False)
                                success = True
                            except Exception as e:
                                self.logger.error(f"Scorer failed to score the response, retrying: {e}")
                    if score > prev_score:
                        ...
                        while not_scussed:
                            try:
                                strategy, summarizer_system = self.summarizer.summarize(request, prev_jailbreak_prompt, jailbreak_prompt, current_library, max_tokens=10000, do_sample=True, temperature=0.6, top_p=0.9)
                                json_formatted_strategy = self.summarizer.wrapper(strategy, max_tokens=2000, do_sample=True, temperature=0.6, top_p=0.9)
                                jailbreak_strategy = json.loads(json_formatted_strategy)
                                not_scussed = False
                            except:
